...
- [x] **Phase 4: Umbrella chart + migration des 9 apps** - `charts/arr-stack/` umbrella avec deps `bjw-s/app-template`, migration des 9 ArgoCD Apps de my-kluster vers 1 seule App, Renovate `customManagers` validé bout-en-bout (functionally complete — 8/9 plans + umbrella deployed in production per CLAUDE.md; plan 04-09 selfHeal+prune re-enable follow-up deferred to v0.3.0)
...
- [x] 05-08-PLAN.md — Wave 5: Cluster apply via auto-tag → my-kluster Renovate → ArgoCD sync; SC#4 anime smoke (checkpoint:human-action) + SC#5 idempotence dispositive + SC#6 configarr 3-profile verify + post-apply snapshot + diff against Wave 0 baseline (completed 2026-05-16 — 7 deviations documented in 05-08-SUMMARY.md, all 6 SC dispositives green)
**UI hint**: yes
...
**Plans**: 6 plans
- [x] 07-01-PLAN.md — Wave 0: pre-write Jellyfin snapshot baseline (ADR-6) + Q9 PUT-probe evidence file + JELLYFIN_API_KEY bootstrap operator checkpoint
- [x] 07-02-PLAN.md — Wave 1: pydantic resource models (resources/jellyfin/) + RootConfig.jellyfin + Settings.jellyfin_api_key + JSON Schema regen (Pitfall 6 type-layer enforcement)
