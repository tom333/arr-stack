...
- **D-08:** configarr `--dry-run` availability in v1.28.0 is **unconfirmed** (carry-forward blocker from STATE.md). Research MUST find a configarr mode that validates the YAML **without connecting to live Sonarr/Radarr** (parse / schema / validate-only). If such a mode exists → use it as the CI gate. **If NO such mode exists → STOP and escalate options to the user before settling.** Do NOT silently fall back to pydantic-only validation. (Ephemeral-*arr-in-CI was considered and rejected as the default — only revisit if escalation forces it.)
