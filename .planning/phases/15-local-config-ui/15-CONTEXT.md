...
### Schema-driven form (operator clarification, post-discussion amendment)

- **D-13:** **The UI is a schema-driven form, NOT a YAML text editor.** The JSON Schema generated by pydantic (`schemas/arrconf-schema.json` — already committed, updated via `arrconf schema-gen` per Phase 12 D-05) is the SINGLE SOURCE OF TRUTH for:
  1. **Input widget choice per field**, walked from `$defs.<TypeName>.properties.<field>.type` + `.enum` + `.format`:
     - `type: string` + `enum: [...]` → `<select>` dropdown (the 4 known Literals: `kind`, `profile`, `apps.type`, `apps.sync_level`).
     - `type: string` (no enum) → `<input type="text">` (with `pattern` attribute if `format: ipv4`, `uri`, etc.).
     - `type: integer` → `<input type="number">` (with `min`/`max`/`step` if present in schema).
     - `type: boolean` → `<input type="checkbox">`.
     - `type: array` → repeatable list editor (Categories table is one specialization; `indexers.items[]`, `notifications.items[]`, etc. follow the same pattern).
     - `type: object` → nested form (collapsible per Surface 3 pattern).
     - `$ref` → resolved into nested form using the referenced `$defs` entry.
  2. **Tooltip / help text per field**, from `properties[field].description`. The 54 `Field(description="...")` strings in `tools/arrconf/arrconf/config.py` surface as inline help next to each input (subtle `(i)` icon → hover/click tooltip; mobile-style touch-friendly even though we're laptop-only).
  3. **Client-side validation hints** (CONTEXT D-06 "cheap client-side checks"): required vs optional (`required[]` array), min/max bounds, regex patterns, enum membership. These are immediate visual feedback before Save. Backend pydantic remains the source of truth on Save.
  4. **Section grouping**: the JSON Schema's top-level `properties` keys drive the per-app collapsible sections; the order in the schema dictates the order in the UI.

  The frontend has a generic `FieldInput.svelte` component that takes `(schema_node, value, on_change)` and renders the right widget. There's no per-field hardcoded UI — adding a new pydantic field to `config.py` and re-running `arrconf schema-gen` makes it appear automatically in the UI.

  **Operator value**: this turns the UI into a **discovery + learning aid**, not just a save mechanism. New operator opens the UI, sees a tooltip on `seerr.main.sonarr_service.activeAnimeProfileId` reading "Sonarr quality profile integer ID for anime (Phase 5 'Anime' profile)", picks the right value via the schema-hinted constraints, saves.

- **D-14:** **Inline help is IN scope** (this AMENDS the original D-10 "full help system out of scope" — that deferred item is rescinded per operator clarification). The implementation is the JSON-Schema-driven tooltip pattern in D-13 above. NO additional documentation framework, NO Markdown rendering — just the pydantic descriptions surfaced as tooltips. If a field has an empty / generic description in `config.py`, that's a follow-up to enrich the description string at the source (Python file), not a separate help system in the UI.
