<script lang="ts">
  ...
  let categories = $state<SeriesCategory[]>([]);
  let grabbing = $state<Release | null>(null);   // release awaiting category choice
  let grabCategory = $state("");                  // selected category name
  let monitor = $state("all");                    // monitoring strategy
  let grabBusy = $state(false);
  let grabError = $state<string | null>(null);

  const PROFILES = ["MULTi.VF", "Anime", "Family"];
  const DEFAULT_CATEGORY = "series";
  const MONITOR_OPTIONS: { value: string; label: string }[] = [
    { value: "all", label: "Toute la série" },
    { value: "latestSeason", label: "Saison récente + futurs" },
    { value: "future", label: "Futurs seulement" },
  ];
  ...
  async function load() {
    ...
    error = null;
    try {
      items = await getSeriesReleases(profile);
    } catch (e) {
  ...
  async function forceRefresh() {
    await refreshSeriesReleases();
    await load();
  }

  function openGrab(r: Release) {
    if (r.tmdb_id == null) {
      error = `Pas de TVDB résolu pour ${r.title}`;
      return;
    }
    grabError = null;
  ...
  async function confirmGrab() {
    ...
    try {
      await addSeries({
        tvdb_id: r.tmdb_id, title: r.title, year: r.year,
        category: grabCategory, monitor,
      });
      grabbing = null;
      await load();
    } catch (e) {
      ...
      <tbody>
        {#each visible as sr (sr.release.info_hash)}
          ...
          >
            ...
            <td class="mono">{sr.release.episode_label ?? "—"}</td>
            <td>
    ...
    <div class="modal" onclick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
      ...
      <label>Monitoring
        <select bind:value={monitor}>
          {#each MONITOR_OPTIONS as m}<option value={m.value}>{m.label}</option>{/each}
        </select>
      </label>
